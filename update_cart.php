...
if (isset($_POST['update_keranjang'])) {
    ...
    foreach ($id_pesanan_arr as $indek => $id_pesanan) {
        $id_pesanan = trim($id_pesanan); 
        // bersihkan spasi jika ada
        $qty = (int)$qty_arr[$indek];

        // Ambil harga produk
        $stmt_harga = $koneksi->prepare("SELECT pr.harga FROM tb_pesanan p JOIN tb_produk pr ON p.id_produk =pr.id_produk WHERE p.id_pesanan = ? LIMIT 1");
        $stmt_harga->bind_param("s", $id_pesanan);
        // <- gunakan 's' untuk string
        $stmt_harga->execute();
        $result_harga = $stmt_harga->get_browserresult();

        if ($row_harga = $result_harga->get_fetch_assoc()) {
